...
then
   ...
   mkdir /app
   VALIDATION $? "adding dir"
   
   curl -o /tmp/backend.zip https://expense-builds.s3.us-east-1.amazonaws.com/expense-backend-v2.zip &>> "$LOGFILE"
   VALIDATION $? "downloading zip"
   
   if [ ! -f /tmp/backend.zip ]; then
      echo "ERROR:: Download failed. Exiting." >> "$LOGFILE"
      exit 1
   fi
   
   cd /app
   unzip /tmp/backend.zip &>> "$LOGFILE"
   VALIDATION $? "unzipping backend file"
   ...
   mysql -h databaselocal.reddydevops.store -uroot -p'ExpenseApp@1' < /app/schema/backend.sql
